    ...
    match err {
        crate::error::Error::FigmaImportError(NetworkError(network_error)) => {
            ...
            if let Some(status) = network_error.status() {
                match status.as_u16() {
                    400 => env.throw_new(
                        jni::strings::JNIString::from("com/android/designcompose/FetchException"),
                        jni::strings::JNIString::from(format!("Bad request: {}", status)),
                    )?,
                    403 => env.throw_new(
                        jni::strings::JNIString::from(
                            "com/android/designcompose/AccessDeniedException",
                        ),
                        jni::strings::JNIString::from("Invalid Authentication Token"),
                    )?,
                    404 => env.throw_new(
                        jni::strings::JNIString::from(
                            "com/android/designcompose/FigmaFileNotFoundException",
                        ),
                        jni::strings::JNIString::from(doc_id),
                    )?,
                    429 => env.throw_new(
                        jni::strings::JNIString::from(
                            "com/android/designcompose/RateLimitedException",
                        ),
                        jni::strings::JNIString::from("Figma Rate Limit Exceeded"),
                    )?,
                    500 => env.throw_new(
                        jni::strings::JNIString::from(
                            "com/android/designcompose/InternalFigmaErrorException",
                        ),
                        jni::strings::JNIString::from("Figma.com internal error"),
                    )?,
                    code => env.throw_new(
                        jni::strings::JNIString::from("com/android/designcompose/FetchException"),
                ...
                env.throw_new(
                    jni::strings::JNIString::from("java/net/ConnectException"),
                    jni::strings::JNIString::from(format!("Network error: {}", network_error)),
                )?
            } else {
                env.throw_new(
                    jni::strings::JNIString::from("java/net/SocketException"),
                    jni::strings::JNIString::from(format!("Network error: {}", network_error)),
                )?
            }
        }
