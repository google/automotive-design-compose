...
use crate::{Node, transform_flexbox, View};
use crate::component_context::ComponentContext;
use crate::extended_layout_schema::{ExtendedAutoLayout, LayoutType, SizePolicy};
use crate::figma_schema::{Component, ComponentSet, EffectType, HorizontalLayoutConstraintValue, LayoutAlign, LayoutAlignItems, LayoutMode, LayoutSizing, LayoutSizingMode, LineHeightUnit, NodeData, OverflowDirection, StrokeAlign, TextAlignHorizontal, TextAlignVertical, TextAutoResize, TextTruncation, VerticalLayoutConstraintValue};
use crate::image_context::ImageContext;
use crate::reaction_schema::{FrameExtras, Reaction, ReactionJson};
use crate::toolkit_font_style::{FontStyle, FontWeight};
use crate::toolkit_layout_style::layout_style::LayoutStyle;
use crate::toolkit_layout_style::{AlignContent, AlignItems, AlignSelf, Dimension, FlexDirection, FlexWrap, JustifyContent, Overflow, PositionType};
use crate::toolkit_schema::{ComponentInfo, RenderMethod, ScrollInfo, ViewShape};
use crate::toolkit_style::{FilterOp, FontFeature, GridLayoutType, GridSpan, ItemSpacing, LayoutTransform, LineHeight, MeterData, ShadowBox, StyledTextRun, TextAlign, TextOverflow, TextStyle};

// Map Figma's new flexbox-based Auto Layout properties to our own flexbox-based layout
// properties.
pub fn compute_layout(node: &Node, parent: Option<&Node>) -> LayoutStyle {
...
pub fn convert_transform(transform: &crate::figma_schema::Transform) -> LayoutTransform {
...
fn visit_node(
    ...
    */
    ...
    style.blend_mode = transform_flexbox::convert_blend_mode(node.blend_mode);
    ...
    } = &node.data
    {
        if let Some(text_fill) = node.fills.iter().filter(|paint| paint.visible).last() {
            style.text_color = transform_flexbox::compute_background(text_fill, images, &node.name);
        }
        ...
        */
        ...
        if character_style_overrides.is_empty() {
            ...
            let mut flush = |current_run: &mut String, last_style: &mut Option<usize>| {
                ...
                {
                    let substyle_fill = transform_flexbox::compute_background(text_fill, images, &node.name);
    ...
    for fill in node.fills.iter().filter(|paint| paint.visible) {
        style.background.push(transform_flexbox::compute_background(fill, images, &node.name));
    }

    for stroke in node.strokes.iter().filter(|paint| paint.visible) {
        style.stroke.strokes.push(transform_flexbox::compute_background(stroke, images, &node.name));
    }
    ...
    let fill_paths = if let Some(fills) = &node.fill_geometry {
        fills.iter().filter_map(transform_flexbox::parse_path).collect()
    } else {
        Vec::new()
    };
    ...
    let stroke_paths = if let Some(strokes) = &node.stroke_geometry {
        strokes.iter().filter_map(transform_flexbox::parse_path).collect()
    } else {
        Vec::new()
    };
