...
data class Customization(
    ...
    var brushState: Optional<State<Brush>> = Optional.empty(),
...
fun CustomizationContext.setBrushState(nodeName: String, brushState: State<Brush>) {
    customize(nodeName) { c -> c.brushState = Optional.of(brushState) }
}
...
fun CustomizationContext.getBrushState(nodeName: String): State<Brush>? {
    val c = cs[nodeName] ?: return null
    if (c.brushState.isPresent) return c.brushState.get()
    return null
}
