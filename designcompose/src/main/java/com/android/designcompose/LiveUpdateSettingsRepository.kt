...
class LiveUpdateSettingsRepository(private val dataStore: DataStore<Preferences>) {
    ...
    private val adaptivePollingEnabledPrefKey = booleanPreferencesKey("AdaptivePollingEnabled")
    ...
    internal val adaptivePollingEnabledFlow: Flow<Boolean> =
        dataStore.data
            .catch { exception ->
                if (exception is IOException) {
                    Log.e(liveUpdateTag, "Error reading LiveUpdate Settings.", exception)
            ...
            .map { settings -> settings[adaptivePollingEnabledPrefKey] ?: true }
    ...
    internal val liveUpdateTimeoutFlow: Flow<Long> =
        dataStore.data
            ...
            .map { settings ->
                settings[liveUpdatePauseMillisPrefKey] ?: DocServer.DEFAULT_LIVE_UPDATE_PAUSE_MILLIS
            }
    ...
    internal val liveUpdateFetchMillisFlow: Flow<Long> =
        dataStore.data
            ...
            .map { settings ->
                settings[liveUpdateFetchMillisPrefKey] ?: DocServer.DEFAULT_LIVE_UPDATE_FETCH_MILLIS
            }
    ...
    suspend fun setAdaptivePollingEnabled(enabled: Boolean) {
        dataStore.edit { it[adaptivePollingEnabledPrefKey] = enabled }
        Log.i(TAG, "Adaptive polling enabled set to $enabled")
    }
}
