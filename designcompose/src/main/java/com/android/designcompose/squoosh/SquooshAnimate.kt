...
internal fun createMergedAnimationTree(
    ...
    val (cloned: SquooshResolvedNode, alreadyBuiltChildren) =
        if (requestedAnim != null && requestedAnim.animationControl == null) {
            ...
            if (toNode != null) {
                ...
                val mergedClonedNode = mergeRecursive(from, toNode, parent, animations)
    ...
    if (nextFrom != null) {
        val nextCloned = createMergedAnimationTree(nextFrom, to, requestedAnimations, parent)
    ...
    if (!alreadyBuiltChildren) {
        ...
        if (childFrom != null) {
            val child = createMergedAnimationTree(childFrom, to, requestedAnimations, cloned)
...
private fun mergeRecursive(
    ...
    if (isTweenable(from.view, to.view) && !needsStyleTween(from.style, to.style)) {
        ...
        while (child != null) {
            val matchingFromChild = findChildNamed(from, child.view.name)
            if (matchingFromChild != null) {
                ...
                val c = mergeRecursive(matchingFromChild, child, n, anims)
...
private fun findChildNamed(parent: SquooshResolvedNode, name: String): SquooshResolvedNode? {
    var child = parent.firstChild
    while (child != null) {
        if (child.view.name == name) return child
        child = child.nextSibling
    }
    return null
}
