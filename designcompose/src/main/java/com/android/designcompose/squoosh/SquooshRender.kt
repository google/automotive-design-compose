...
import com.android.designcompose.definition.element.strokeWeightOrNull
...
private fun squooshTextRender(
    ...
    val strokeWidth =
        (style.nodeStyle.strokeOrNull?.strokeWeightOrNull?.toUniform() ?: 0f) * density.density
    // Only drop shadow is supported now.
    val shadow = style.nodeStyle.textShadowOrNull
    val blurRadius = (shadow?.blurRadius ?: 0f) * density.density
    val xOffset = (shadow?.offsetX ?: 0f) * density.density
    val yOffset = (shadow?.offsetY ?: 0f) * density.density
