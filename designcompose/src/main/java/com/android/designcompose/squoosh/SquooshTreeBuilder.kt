...
internal fun resolveVariantsRecursively(
    ...
    if (replacementComponent != null) {
        ...
        skipChildren = true
    } else if (customModifier != null) {
        // The node has a registered Modifier. Mark needsChildRender so the parent draw
        // defers to the composable, where the modifier chain wraps the rendered pixels.
        composableList?.addChild(
            SquooshChildComposable(
                node = resolvedView,
                parentComponents = parentComps,
