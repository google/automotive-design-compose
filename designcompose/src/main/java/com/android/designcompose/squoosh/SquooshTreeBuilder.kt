...
internal fun resolveVariantsRecursively(
    ...
    if (v.hasComponentInfo()) {
        parentComps = ParentComponentData(parentComponents, v.id, v.componentInfo)
        ...
        if (v.componentInfo.hasOverrides()) {
            val overrides = v.componentInfo.overrides
        ...
        interactionState.squooshNodeVariant(v.id, customizations.getKey(), document)?.let {
            view = it
        }
        ...
        if (view.name == v.name) { // TODO: why we don't check if view == v????
            ...
            val variantNodeName =
                variantTransition.selectVariant(
                    v.id,
                    customizations.getMatchingVariant(v.componentInfo) ?: v.componentInfo.name,
                )
            if (variantNodeName != null && variantNodeName != v.componentInfo.name) {
                // Find the view associated with the variant name
                val variantNodeQuery =
                    NodeQuery.NodeVariant(
                        variantNodeName,
                        variantParentName.ifEmpty { v.componentInfo.componentSetName },
                    )
                val variantView =
    ...
    val layoutId = computeLayoutId(componentLayoutId, v.uniqueId)
...
internal fun generateReplacementListChildNode(
    ...
    val listChildView = view {
        ...
        data = viewData { this.container = listChildViewData }
