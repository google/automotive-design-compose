...
internal fun Background.asBrush(
    ...
    if (hasImage()) {
        ...
        val imageTransform = backgroundImage.transformOrNull?.asSkiaMatrix()
        if (DebugNodeManager.getUseLocalRes().value && backgroundImage.hasResName()) {
            val resId =
                appContext.resources.getIdentifier(
                    backgroundImage.resName,
                    "drawable",
            ...
            if (resId != Resources.ID_NULL) {
                ...
                return Pair(
                    RelativeImageFill(
                        ...
                        scaleMode = backgroundImage.scaleMode,
                    ),
                    backgroundImage.opacity,
                )
            } else {
                Log.w(TAG, "No drawable resource ${backgroundImage.resName} found")
            }
        }
    ...
    if (hasLinearGradient()) {
        val linearGradient = getLinearGradient()
    ...
    if (hasRadialGradient()) {
        val radialGradient = getRadialGradient()
        return when (radialGradient.colorStopsCount) {
            0 -> {
    ...
    if (hasAngularGradient()) {
        val angularGradient = getAngularGradient()
        return when (angularGradient.colorStopsCount) {
            0 -> {
