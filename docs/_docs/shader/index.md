...
4. **Clear all**: Choose the clear all option to remove all the shaders from the current file.

### Supported Uniforms

Uniforms are variables that can be used by the shaders. We don't parse user's input so all the uniforms
need to be created using our `+ Uniform` button.

<img src="CreateUniforms.png">
...
- float: A single floating-point number.
  - iTime: A special float uniform that simulates time. We ask user to only create it when it is used
    ...
    necessary to avoid unnecessary redraw.
...
- **API Level:** [Android Graphics Shading Language (AGSL)][2] only works on Android 13 and above. We
  provide an option to use a pinky color as the background/stroke color on unsupported Android devices.
...
We provide an infinite simulation using `withInfiniteAnimationFrameMillis` which provides a time
source for display frames starting from value `0.0f`. See [ShaderHelper.kt][3]. Our implementation:
...
- Allows for configuring the frame update interval via the [`config_shader_frame_interval_ms` resource][4]
  ...
  interval results in fewer updates. This interval is set to ensure optimal performance on lower-end
  Android devices.

- Multiplies the global `ANIMATOR_DURATION_SCALE` setting with the frame interval and can disable
  the shader animation if the user sets a Developer Option to set the animator duration scale
  to 0.
...
Image(child shader) uniform can be customized using a drawable resource in the following way:
...
But it is hacky and it has unexpected result when toggle the `Use local resources` option in design
switcher. We will look into the image uniform customization in the short future.
...
No offical support for image uniform state customizations.
...
We support different versions of the design. If the same node in different design docs use different
shaders and different set of shader uniforms, it can include the joint set of the uniform customizations.
Those uniforms missing from the design doc will be ignored.
...
- **Contact Support:** If problems persist, [get in touch][6] with our team for support.
...
[6]: https://github.com/google/automotive-design-compose?tab=readme-ov-file#get-in-touch
