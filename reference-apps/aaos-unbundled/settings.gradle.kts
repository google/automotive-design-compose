...
To use a pre-built DesignCompose SDK: Set the DesignComposeMavenRepo Gradle Property
to the path to the pre-built Maven directory.
To use the SDK from the local source, set DesignComposeMavenRepo like above and also
set alwaysRebuildDesignComposeSdk (to anything). The SDK will be built by a task
in the buildSrc project, which ensures the repository exists before the root project
checks for it.
 */
