...
impl Easing {
    ...
    pub fn apply(&self, t: f32) -> f32 {
        match self {
            ...
            Easing::Instant => if t >= 1.0 { 1.0 } else { 0.0 },
            Easing::EaseIn => t * t,
            Easing::EaseOut => t * (2.0 - t),
            Easing::EaseInOut => if t < 0.5 { 2.0 * t * t } else { -1.0 + (4.0 - 2.0 * t) * t },
            Easing::EaseInCubic => t * t * t,
            Easing::EaseOutCubic => { let t = t - 1.0; t * t * t + 1.0 },
            Easing::EaseInOutCubic => if t < 0.5 { 4.0 * t * t * t } else { let t = t - 1.0; 4.0 * t * t * t + 1.0 }, // Corrected math
        }
    }
}
...
impl KeyframeValue {
    ...
    pub fn interpolate(&self, other: &KeyframeValue, t: f32) -> KeyframeValue {
        match (self, other) {
            ...
            (KeyframeValue::Arc(a), KeyframeValue::Arc(b)) => {
            ...
            (KeyframeValue::Gradient(a), KeyframeValue::Gradient(b)) => {
                if a.len() != b.len() { return KeyframeValue::Gradient(a.clone()); }
                let stops = a.iter().zip(b.iter()).map(|(s1, s2)| {
                    GradientStop {
...
impl ParsedTimelineData {
    ...
    pub fn parse(encoded_data: &str) -> Result<Self, String> {
        ...
        if parts.is_empty() { return Err("Empty data".to_string()); }
        ...
        if parts.len() > 1 && !parts[1].is_empty() {
            for kf_str in parts[1].split(';') {
                ...
                if kf_parts.len() != 3 { continue; }
                
                let fraction: f32 = kf_parts[0].parse().unwrap_or(0.0);
                let easing = parse_easing(kf_parts[2]);
                
                // Decode Value
                let decoded_bytes = general_purpose::STANDARD.decode(kf_parts[1]).map_err(|e| e.to_string())?;
        ...
        // Ensure sorted
        keyframes.sort_by(|a, b| a.fraction.partial_cmp(&b.fraction).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub fn interpolate(&self, start_value: &KeyframeValue, end_value: &KeyframeValue, fraction: f32) -> KeyframeValue {
        if fraction <= 0.0 { return start_value.clone(); }
        if fraction >= 1.0 { return end_value.clone(); }
        ...
        if let Some(first) = self.keyframes.first() {
            if fraction < first.fraction {
                ...
                for i in 0..self.keyframes.len()-1 {
                    if fraction >= self.keyframes[i].fraction && fraction < self.keyframes[i+1].fraction {
        ...
        if duration <= 0.00001 { return kf2_value.clone(); }
...
fn parse_easing(s: &str) -> Easing {
    match s {
        ...
        _ => Easing::Linear, // Default/Inherit -> Linear (simplified)
    }
}
...
fn parse_keyframe_value(json_str: &str) -> KeyframeValue {
    ...
    #[derive(Deserialize)]
    struct RawStop { position: f32, color: String }
    if let Ok(raw_stops) = serde_json::from_str::<Vec<RawStop>>(json_str) {
        let stops = raw_stops.into_iter().map(|s| GradientStop {
            position: s.position,
            color: Rgba::from_hex(&s.color)
        }).collect();
    ...
    pub fn new(variant: &Variant) -> Self {
        ...
        if let Some(anim) = &variant.animation {
            if let Some(data) = &anim.custom_keyframe_data {
                for (key, val) in data {
                    if let Some((node, prop)) = key.rsplit_once('-') {
                        if let Ok(parsed) = ParsedTimelineData::parse(val) {
                            timelines.entry(node.to_string())
...
mod tests {
    ...
    fn test_optimized_interpolation() {
        ...
        let encoded_radii = format!("Linear|0.5,{},Linear", general_purpose::STANDARD.encode("[50.0, 50.0, 50.0, 50.0]"));
